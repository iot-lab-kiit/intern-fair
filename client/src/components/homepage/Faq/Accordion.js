...
const AccordionItem = ({ question, answer, isOpen, onClick }) => {
  ...
  className={`w-full text-left py-5 px-2 flex items-center justify-between text-lg focus:outline-none ${
    isOpen
      ? "text-secondary bg-gradient-to-r from-transparent via-rgba(0,0,0,0.04) to-transparent"
      : ""
...
>
  ...
  <span className="flex flex-shrink-0">  {isOpen ? (
      <AiOutlineMinus className="text-secondary text-2xl" />
    ) : (
      ...
      >
        <p className="pt-2 pb-5 mbXSmall:max-mbMedSmall:text-base mbMini:max-mbXSmall:text-sm max-mbMini:text-xs font-Gilroy-Light">{answer}</p>
      </div>
    </div>
  );
};
