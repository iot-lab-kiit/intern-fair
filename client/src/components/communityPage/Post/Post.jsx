    ...
    return (
        ...
        <div className="p-2 mbSmall:p-4 border-[1.5px] border-[#DCDCE7] min-w-[400px] rounded-lg ml-0 lg:ml-12 sm:w-4/5">
            <div className=" flex flex-col items-start justify-center gap-3 mbMedSmall:gap-4 border-[#E7E8EC] border-b-2 p-4">
                ...
                {/* {console.log(tag)} */}
                <div className="flex items-center justify-center flex-wrap gap-1 mbMedSmall:gap-2 mbSmall:gap-3 " suppressHydrationWarning>
                    {tag &&
                        tag.length > 0 &&
                        tag.map((t, index) => (
                ...
                {image && (
                    <div className="w-full">
                        <span className="w-full h-[8rem] mbXSmall:h-[10rem] mbMedSmall:h-[12rem] mbSmall:h-[14rem] mbMedium:h-[16rem] laptop:h-[18rem] tbPortrait:h-[20rem]  inline-block relative">
                            <Image src={`http://43.204.145.188/assets/${image}`} fill alt="about" className="object-cover" />
                        </span>
                    </div>
                )}
            </div>
            <div className="flex items-center p-4 justify-between">
                <div className="flex items-start justify-center gap-6">
                    <div className="flex items-center justify-center gap-2" onClick={() => handleLikes(id, likes)}>
                        {/* <GoHeart size={32} color="#FF0000" /> */}
                        {/* <FcLike size={32} color="#00FF00" /> */}
                        <p className="text-[#191717] text-sm mbSmall:text-base mbMedium:text-lg">{likes}</p>
                    </div>
